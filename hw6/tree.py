...
class Tree(object):
    ...
    def create_pcfg_a1(self, rules):
        ...
        for daughter in self:
            is_terminal = Tree.terminal(daughter)
            if not is_terminal:
                rules += daughter.create_pcfg(rules)

        return rules

    def pretty_pcgf_a1(t_prod):

        rules = ''

        # Remove duplicate production rules
        #t_prod = list(set(t_prod))
        #t_prod = [list(r) for r in dict(t_prod).items()]
        #t_prod = list(set(map(tuple, t_prod)))

        # Iterate thru a list
        for rule in t_prod:
            mother, daughters = rule
            rules += format('{: <20} -> {}\n'.format(mother, ' '.join(daughters)))

            for (m, ds) in t_prod:
                if m == mother and ds == daughters:
                    next
                elif m == mother and ds != daughters:
                    rules += format('{: <20} -> {}\n'.format(m, ' '.join(ds)))
                else:
                    next
    ...
    def pretty_pcgf_a2(t_prod):
        ...
        # Iterate thru a list
        for production in t_prod:
            mother, daughters = production
            rule = format('{: <20} -> {}\n'.format(mother, ' '.join(daughters)))
    ...
    def pretty_pcgf_a3(t_prod):
        ...
        # s = t_prod
        nonterminals = collections.defaultdict(list)
        for k, v in t_prod:
            nonterminals[k].append(v)
    ...
    def pretty_pcgf_a4(cfg):
        ...
        # for each rule in our context-free grammar
        for rule in cfg:
            # calculate the probabiity of each rule
            # number of rules for nonterminal

            nt, rhs = rule
            #p = 1 / len(rhs)

            # # for each term in the rule
            # for term in rhs:
            #     #v.append(str(p))
            #     nonterminals[nt].append(term)

            #

            #p = 1 / cfg.count(rule)
            #p_rule = { 'rhs': rhs, 'probability': p }
            #nonterminals[nt].append(p_rule)
            nonterminals[nt].append(rhs)

            #x = len(nonterminals[nt])
            #print('x: ' + str(x))
            #p = 1 / len(nonterminals[nt])
            #nonterminals[nt]['probability'] = p
            #nonterminals[nt][1] = p
    ...
    def pretty_pcgf4(cfg):
        ...
        #nonterminals = collections.defaultdict(list)
        nonterminals = collections.defaultdict(lambda: [0, 0])
        ...
        for nt, rhs in cfg:
            ...
            #p_rule = { 'rhs': rhs, 'probability': p }
            #nonterminals[nt].append(p_rule)
            nonterminals[nt].setdefault('probability', 0)
            nonterminals[nt].setdefault('rhs', rhs)
            nonterminals[nt]['probability'] += 1
